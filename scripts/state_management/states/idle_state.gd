...
func execute() -> void:
	...
	if wander_target != Vector2.ZERO:
		character.target_position = wander_target
...
func on_enter() -> void:
	print("[%s] Entering Idle State" % character.character_name)
	wander_start = character.position
	_generate_wander_target()

func on_exit() -> void:
	print("[%s] Exiting Idle State" % character.character_name)
...
func _generate_wander_target() -> void:
	...
	wander_target = character.global_position + random_offset
...
func _try_find_work() -> void:
	...
	if work_request != null:
		...
		character.target_position = work_request.position
